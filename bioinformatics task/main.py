...
def getAverage_getConsenesus():
    Lines=[]
    with open("averages & consenesus.txt","w") as f:
        for seq_record in SeqIO.parse("gisaid_hcov-19_2021_12_31_12.fas", "fasta"):
                vertical_Seq=[]
...
def comparing_sequences():
    ...
    with open("comparing results.txt","w") as f:
        f.write("the indices of different bases of omecron are: "+ "\n")
        for seq_record in SeqIO.parse("gisaid_hcov_omicron-19_2021_12_31_12.fas", "fasta"):
            Lines.append(str(seq_record.seq))
            mysequence=str(seq_record.seq)
        vertical_Seq=[]
        vertical_sequences_list=[]
